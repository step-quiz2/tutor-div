...
st.markdown(
    ...
    /* Marca invisible per enganxar (sticky) la columna de senyals. */
    .signals-sticky-marker { display: none; }
    </style>
    """,
...
# Objectiu: donar més amplada a la conversa i al panell de senyals, i —sobre
# tot— evitar que el panell "Senyals en directe" desaparegui en fer scroll.
# El fem ENGANXAT (position: sticky) perquè segueixi la conversa i sigui
# sempre visible sense haver de pujar manualment. (En mode alumne no hi ha
# panell ni columnes, així que aquests retocs no s'apliquen.)
if DOCENT_MODE:
    st.markdown(
        """
        <style>
        ...
        /* La columna que conté el panell de senyals queda ENGANXADA a dalt:
           es manté visible mentre es fa scroll de la conversa. Si el panell
           fos més alt que la pantalla, té scroll propi. */
        [data-testid="stColumn"]:has(.signals-sticky-marker) {
            position: sticky;
            top: 1.5rem;
...
def render_inspector(state):
    ...
    st.markdown(
        '<span class="signals-sticky-marker"></span>'
